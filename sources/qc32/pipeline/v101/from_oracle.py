...
class FromOracle(Oracle):
	...
	def __init__(self,parent,log,datadir,conf,db):
		...
		if self.args.from_table:
			...
			assert not self.args.query_sql_dir, 'Set Table/Partition or QueryDir but not both.'
			assert not self.args.from_table_list, 'Set Table/Partition or "Table List" but not both.'
			self.T=True
			self.from_table.append(self.args.from_table)
			if hasattr(args, 'from_partition') and self.args.from_partition:
				...
				self.P=True				
				self.from_partition.append(self.args.from_partition.upper())
			elif hasattr(args, 'from_partition_list') and self.args.from_partition_list:				
				self.PL=True
				
				self.from_partition=self.args.from_partition_list.upper().split(',')
				#print hasattr(args, 'from_partition_list') , self.args.from_partition_list
				#e(0)
			elif hasattr(args, 'from_sub_partition') and self.args.from_sub_partition:				
				self.SP=True
				self.from_sub_partition.append(self.args.from_sub_partition.upper())
			elif hasattr(args, 'from_sub_partition_list') and self.args.from_sub_partition_list:				
				self.SPL=True
				self.from_sub_partition=self.args.from_sub_partition_list.upper().split(',')
			self.from_partition=list(set(self.from_partition))
			self.from_sub_partition=list(set(self.from_sub_partition))	
		elif self.args.from_table_list:
			assert not self.args.from_table, 'Set Table/Partition or "Table List" but not both.'
			assert not self.args.query_sql_file, 'Set "Table List" or Query but not both.'
			assert not self.args.query_sql_dir, 'Set "Table List" or QueryDir but not both.'
			
			self.TL=True
			self.from_table=self.args.from_table_list.split(',')
		else:
		...
		if self.V:
			if self.T:
				...
				if self.P or self.PL or self.SP:
...
SELECT %s FROM (%s) %s %s;\nexit;\n""" % (llen,feedback,  cl, qry, ' ',orderby)
	...
	def print_copy_details(self):
		if self.T:
			source='table: %s' % self.from_table[0]
		elif self.QF:
	...
	partition: %s""" % self.args.from_partition
		if self.PL:
			part="""
	partition list: %s""" % self.args.from_partition_list	
		if self.SP:
			part="""
	sub-partition: %s""" % self.args.from_sub_partition		
		if self.SPL:
			part="""
	sub-partition list: %s""" % self.args.from_sub_partition_list		
		print """		
From %s:	
	from db: %s
	%s%s
