...
class FromJSON(JSON):
	"""JSON file"""
	def __init__(self,parent,log,input_files,datadir,conf, db='JSON'):
		self.log=log
		self.input_files=[]
		...
		if hasattr(args, 'input_files') and args.input_files:
			...
			for dir in self.dirs:
				...
				for df in os.listdir(dir):
					#print qf
					self.input_files.append(os.path.join(dir, df))
					self.input_file_names.append(df)	
		#print self.input_file
...
%s
	...
	def set_file_payload(self, num_of_shards):
		...
		for i in range(len(self.shards)):
			...
			(sid,row_from, row_to, _) = self.shards[i]
			if 1:
				from_pld.append(('Shard-%s' % i,self.input_files[0], row_from, row_to,self.field_term))
	...
	def set_filelist_payload(self, num_of_shards):
		...
		#pprint( from_pld)
		#print self.skip_rows
		if hasattr(self.args, 'skip_rows') and self.args.skip_rows>1:
			self.log.warn('Skipping %s rows', self.args.skip_rows)
