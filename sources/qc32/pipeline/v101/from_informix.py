...
import shlex
import datetime
class FromInformix(Informix):
	"""Informix db"""
	def __init__(self,parent,log,datadir,conf,db):
		(self.args,self.uargs,self.conf)=(conf.args, conf.uargs	,conf)
		self.login=(args.from_user, args.from_passwd, args.from_db_name, args.from_db_server)
		Informix.__init__(self,log,self.login,datadir,args) 
		...
		if self.args.lame_duck:
			self.lim  = 'FIRST %s' % self.args.lame_duck			
	...
	def get_sharded_sql(self,sql_query,rowid_from,rowid_to, rc,outfn):
		...
		if not rowid_from and not rowid_to:
			...
			if not rowid_from:		
				...
				if rowid_to: 
					from_to=' SKIP %s LIMIT %s ' % (rowid_from, rowid_to)
				else:
					from_to=' SKIP %s ' % (rowid_from)
		hdr=''
		#if self.args.header:
		#	hdr=' WITH COLUMN NAMES'
		out=r"""UNLOAD TO "%s" DELIMITER '%s' SELECT %s * FROM (%s) q;
""" % (os.path.normpath(outfn).replace("\\", "\\\\"),self.args.field_term,from_to,sql_query.strip().strip(';'))	
		#print out
		#sys.exit(0)
	...
	def set_query_payload(self, num_of_shards):
		...
		sql_query= 'SELECT %s * FROM (%s)' % (self.lim, self.from_query[0])  #f.read()
	...
	def get_table_spool_config(self,from_pld):
		...
		#spConf=[ db_client_loc , '%s@%s' % (self.args.from_db_name, self.args.from_db_server),  sqfn]
		spConf=[ db_client_loc , '%s@%s' % (self.args.from_db_name, self.args.from_db_server)]
	...
	def spool_source_data(self,outfn, spConf, env):
		...
		if 1:
			...
			cqfn=self.save_qry('spool_data_%s_%s' % (shard_name,ts),qry)
			#print cqfn
			p2 = Popen([spConf[0],'-', cqfn], stdin=PIPE, stdout=PIPE, stderr=PIPE)# '-S',  stdin=p1.stdout,
			#print qry
			err,output = p2.communicate()
			#err,output = p2.communicate(qry)
			#print err
			#print output
			status=p2.wait()
		#print output 
