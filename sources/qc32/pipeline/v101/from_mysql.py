...
class FromMysql(Mysql):
	...
	def __init__(self,parent,log,datadir,conf,db):
		...
		Mysql.__init__(self,log,self.login,datadir,self.args) 
		self.log=log		
		#self.login=login
...
From %s:	
	from db: %s
	%s%s
	shards:\t%s""" % (conf.dbs[self.db], '%s/%s/%s' % (self.args.from_user,self.args.from_db_name,self.args.from_db_server),source, part,self.args.num_of_shards)	
	...
	def count_rows(self,query):
		...
		q="""select 'ROW COUNT:' filter ,count(*) cnt from (%s) as t;""" % query.strip().strip(';')
...
#SELECT City2 FROM     Persons_pipe_datetime Order by City2 LIMIT 27,17;
	...
	def set_part_table_payload(self, num_of_shards):
		...
		#sys.exit(1)
		sql_query= 'SELECT * FROM %s PARTITION(%s) %s' % (self.args.from_table, self.args.from_partition,self.lim)
		assert sql_query, 'Query is not set'
		self.rc=self.count_rows(sql_query)
	...
	def set_subpart_table_payload(self, num_of_shards):
		...
		#sys.exit(1)
		sql_query= 'SELECT * FROM %s PARTITION(%s) %s' % (self.args.from_table, self.args.from_sub_partition,self.lim)
		assert sql_query, 'Query is not set'
		self.rc=self.count_rows(sql_query)
		self.log.info(self.rc)
		#self.log.info('Done.')
	...
	def set_table_payload(self, num_of_shards):
		...
		shq='SELECT * FROM %s %s' % (self.args.from_table, self.lim)
		self.rc=self.count_rows(shq)
		self.log.info(self.rc)
		self.log.info('Done.')
		#self.log.info('Fetching query columns.')
		self.log.info('Fetching source column names...')
