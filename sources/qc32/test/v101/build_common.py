...
HostMap=r'".\config\host_map\host_map.py"'
...
class common_build(base):
	...
	def build_test_dbs2(self, relc, tests,cmd):
		...
		for t in tests:
			...
			for element in tests[t]:
				...
				elif 'TableNamedFiles' in db_from and not 'DeleteTargetRecs' in db_to:
					print '1 passing on:', db_from, db_to					
				elif not 'TableNamedFiles' in db_from and 'DeleteTargetRecs' in db_to:
					print '2 passing on:', db_from, db_to		
				elif 'TableNamedFiles' in db_from and not 'GetTabnameFromQuery' in db_to:
					print '3 passing on:', db_from, db_to
				elif 'GetTabnameFromQuery' in db_to and db_from.upper()[:3] in self.conf.ff:
					print '4 passing on:', db_from, db_to	
				elif 'GetTabnameFromQuery' in db_to and not 'QUERY' in db_from.upper():
					print '4 passing on:', db_from, db_to						
				elif  'Table_NoTableName' in db_to and not ('TableList' in db_from or db_from.endswith('Table')):
					print '4 passing on:', db_from, db_to,  'Table_NoTableName' in db_to, not 'TableList' in db_from				
				elif 'NoTable' in db_to and not 'NoTable' in db_from:
					print '5 passing on:', db_from, db_to	
				elif db_to.startswith('DDL') and not ('_Table' in db_from and not 'JSON' in db_from):
					print '61 passing on:', db_from, db_to	
				elif 'Subpartition' in db_from and 'Partition' in db_to:
					print '6 passing on:', db_from, db_to					
				elif 'Partition' in db_from and  'Subpartition' in db_to:
					print '6 2 passing on:', db_from, db_to					
				else:
					...
					if '%s%s%s' % (paramkey_from.lower(),self.conf._to,db_to.lower()) not  in ('csv%scsv' % self.conf._to):
						...
						if 1:
							...
							if 'MONGO' in db_from :
								...
								col_sep='|'
							if db_from.upper().split('_')[0] in self.conf.ff:
								spool_type=''
							elif 'JSON' in db_from.upper() or 'JSON' in db_to.upper():
								spool_type='json'
							else:
								spool_type='csv'
							b=['%s%s%s' % (paramkey_from.upper(),self.conf._to,paramkey_to.upper()),1,			1,				'"%s"' % col_sep,		lame_duck,keep_spool_file,0,0,ask_to_truncate,0,email_to,log_dir,'',job_name,time_stamp,'',job_pre_etl,'','','',loader_profile,HostMap,spool_type,debug_level,status_pipe_id]
			...
			f.close()
			api_todir=os.path.join(github_dir,'args_api',source)
		...
		for t in tests:
			...
			for element in itertools.product(t_from,t_to):
				...
				#	print  '%s does not support limit' % conf.dbs[db_to.upper()]
					...
					print '62 passing on:', db_from, db_to						
				elif 'Subpartition' in db_from and 'Partition' in db_to:
					print '6 passing on:', db_from, db_to					
				elif 'Partition' in db_from and  'Subpartition' in db_to:
					print '6 2 passing on:', db_from, db_to					
	...
	def release_ucon(self,relc,exefn):
		...
		shutil.copytree(ucon_home,ucon_to_dir)	
		test_dir=os.path.join('test','v101','data')
		test_to_dir=os.path.join(reldir,test_dir)
		test_home=os.path.join(self.dmhome,test_dir)
		#print test_home
		#print test_to_dir
		shutil.copytree(test_home,test_to_dir)	
		test_dir=os.path.join('test','v101','query')
		test_to_dir=os.path.join(reldir,test_dir)
		test_home=os.path.join(self.dmhome,test_dir)
		#print test_home
		#print test_to_dir
	...
	def run_all_exe_tests(self,cmd,exefn):
		for c in cmd:
			...
			print ''
			print '-'*50
			print "is_release=False,nor_t=(['%s'],['%s']),run=True)" % (t_from, t_to)
			print '-'*50			
			#e(0)
	...
	def build_test(self, relc, t_from,t_to,cmd):
		...
		for element in itertools.product(t_from,t_to):
			...
			if ('CSV_Sharded' in db_from and db_to in ('SYIQ', 'SYANY', 'SYASE', 'INFORC','INFOR','SLITE','TTEN','MARIA','MYSQL','INFOB','PGRES','DBTAES', 'DBTES', 'DBTAWS', 'DBTWS', 'DBTE', 'DBTEC', 'DBTDE' )):
				...
				print '41 passing on:', db_from, db_to				
			elif  'Table_NoTableName' in db_to and not ('TableList' in db_from or db_from.endswith('Table')):
				print '5 passing on:', db_from, db_to,  'Table_NoTableName' in db_to, not 'TableList' in db_from				
			elif 'NoTable' in db_from and not 'NoTable' in db_to:
				print '63 passing on:', db_from, db_to	
			elif 'Subpartition' in db_from and 'Partition' in db_to:
				print '6 passing on:', db_from, db_to					
			elif 'Partition' in db_from and  'Subpartition' in db_to:
				print '6 2 passing on:', db_from, db_to				
				...
				print '7 passing on:', db_from, db_to						
			else:
				if db_to.startswith('ORA'):
					loader_profile=with_loader_profile
				else:
