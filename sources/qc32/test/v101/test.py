...
data_loc=r'.\test\v101\data'

wide_test=False

ss_keys = [x for x,v in conf.dbfam.items() if v in ['SQL Server']]
if 0:
	...
	if conf.citi:
		...
		csvf['ORA11G'] =csvf['ORA10G'] =csvf['ORA9I'] =csvf['ORA8I'] =csvf['ORA733'] = '%s\\oracle_shard_0_ts.data' % data_loc
...
if conf.citi:
	...
	csvf_tz['ORA11G'] =csvf_tz['ORA10G'] =csvf_tz['ORA9I'] =csvf_tz['ORA8I'] =csvf_tz['ORA733'] = '%s\\oracle_shard_0_tz.data' % data_loc
...
for k in ss_keys:
	csvf_tz[k] = '%s\\ss_shard_0.data' % data_loc
...
if conf.citi:
	...
	csvf_ts['ORA11G'] = csvf_ts['ORA10G'] = csvf_ts['ORA9I'] = csvf_ts['ORA8I'] = csvf_ts['ORA733'] = '%s\\oracle_shard_0_ts.data' % data_loc
...
for k in ss_keys:
	csvf_ts[k] = '%s\\ss_shard_0.data' % data_loc
...
if conf.citi:
	...
	csvf_dt['ORA11G'] = csvf_dt['ORA10G'] = csvf_dt['ORA9I'] = csvf_dt['ORA8I'] = csvf_dt['ORA733'] = '%s\\oracle_shard_0_dt.data' % data_loc
...
for k in ss_keys:
	csvf_dt[k] = '%s\\ss_shard_0.data' % data_loc
#csvf_dt['SSENT'] = '%s\\ss_shard_0.data' % data_loc
csvf_dt['MONGO'] =csvf_ts['MONGO'] = csvf_tz['MONGO'] = '%s\\mongo_shard_0.data' % data_loc

csvf_tz['CURL'] =csvf_dt['CURL'] =csvf_ts['CURL']='http://oatsreportable.finra.org/OATSReportableSecurities-SOD.txt'
...
qryf['ORA11G'] =qryf['ORA10G'] =qryf['ORA9I'] =qryf['ORA8I'] =qryf['ORA733'] = '%s\\oracle_query.sql' % qry_loc
...
ddir['ORAEXA'] = ddir['ORA11G'] = ddir['ORA10G'] = ddir['ORA9I'] = ddir['ORA8I'] = ddir['ORA733'] = ddir['ORA12C'] = ddir['ORAXE'] =os.path.join(data_loc,'ora_data_dir_1')+','+os.path.join(data_loc,'ora_data_dir_2')
#ddir['SSENT'] =  ddir['SSEXP'] = 
for k in ss_keys:
	ddir[k] = os.path.join(data_loc,'ss_data_dir')
...
ddir['CURL'] = os.path.join(data_loc,'curl_data_dir')
...
def create_run_tests(db_from, db_to, test,citi=False):
	...
	large_csvfile=r'"%s\oracle_shard_0_dt_large.data"' % data_loc
	tablenamed_file=r'"%s\SCOTT.Timestamp_test_to.data"' % data_loc
	tablenamed_file_2=r'"%s\SCOTT.Timestamp_test_to_2.data"' % data_loc
	tfrom['CSV_DateFile']= 			( csvfile_dt, None, 0,	1000)
	mongo_file=r'.\test\v101\data\oracle_shard_0_mongo.csv'
	tfrom['CSV_MongoFile']= 			( mongo_file, None, 0,	1000)
	tfrom['CSV_DateFiles']= 			( '%s,%s' % (csvfile_dt,csvfile_dt), None, 0,	1000)
	#tfrom['CSV_LargeFile']= 			( large_csvfile, None, 0,	1000)
	tfrom['CSV_File_TableNamedFile']= 			( tablenamed_file, None, 0,	1000)
	tfrom['CSV_Files_TableNamedFile']= 			( '%s,%s' % (tablenamed_file,tablenamed_file_2) , None, 0,	1000)
	...
	tfrom['CSV_File']= 		( csvfile_ts, None, 0,	1000)
	...
	tfrom['CSV_Dir']= 			( None, data_dir, 0,	1000)
	...
	if 0:
		...
		noclient_connect_string="'(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=localhost)(PORT=1521))(CONNECT_DATA=(SERVER=DEDICATED)(SERVICE_NAME=orcl)))'"
		if citi:
			#from_table='TBL_TEST_FROM'
			#to_table='TBL_TEST_TO'
			ora_user, ora_pwd, ora_sid=('CSMARTBSER', 'Summer15', 'SMARTQ1',)
			noclient_connect_string="'(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=localhost)(PORT=1521))(CONNECT_DATA=(SERVER=DEDICATED)(SERVICE_NAME=orcl)))'"
	src_url=r'http://oatsreportable.finra.org/OATSReportableSecurities-SOD.txt'
	src_url_list='%s,%s' % (src_url,src_url)
	src_url_file=r'C:\Python27\data_migrator_1239_mongo\test\v101\data\url_list.txt'
	src_url_files=r'C:\Python27\data_migrator_1239_mongo\test\v101\data\url_list.txt,C:\Python27\data_migrator_1239_mongo\test\v101\data\url_list.txt'
	src_url_dir =r'C:\Python27\data_migrator_1239_mongo\test\v101\data\URL_Dir_0'
	src_url_dirs =r'C:\Python27\data_migrator_1239_mongo\test\v101\data\URL_Dir_0,C:\Python27\data_migrator_1239_mongo\test\v101\data\URL_Dir_1'
	out_file=r'C:\tmp\curl_out\OATSReportableSecurities-SOD.txt'
	for dbkey in ('CURL',):
		tfrom['%s_Url_To_File' % dbkey]= (out_file, src_url,None, None, None)
		tfrom['%s_Url' % dbkey]= (out_file, src_url,None, None, None)
		tfrom['%s_UrlList' % dbkey]= (None, None,src_url_list, None, None)
		tfrom['%s_UrlFile' % dbkey]= (None, None,None, src_url_file, None)
		tfrom['%s_UrlFiles' % dbkey]= (None, None,None, src_url_files, None)
		tfrom['%s_UrlDir' % dbkey]= (None, None,None, None, src_url_dir)
		tfrom['%s_UrlDirs' % dbkey]= (None, None,None, None, src_url_dirs)
	#print tfrom['CURL_DownloadFile']
	
	for dbkey in ('ORA12C','ORA11G','ORA10G','ORA9I','ORA8I','ORA733', 'ORAEXA', 'ORAXE'):
		...
		tfrom['%s_Table_withHeader' % dbkey]= (None,None,'%s.%s' % (ora_user, from_table), None, None,None,None,None,	 ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,   1,0)
		...
		tfrom['%s_Table_WithWideRows' % dbkey]= (None,None,'%s.%s' % (ora_user, from_table), None, None,	None,None,None, ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,   0,0)
		
		tfrom['%s_JSON_Table' % dbkey]= (None,None,'%s.%s' % (ora_user, from_table), None, None,	None,None,None,ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,   0,0)
		tfrom['%s_Table' % dbkey]= (None,None,'%s.%s' % (ora_user, from_table), None, None,	None,None,None, ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,   0,0)
		
		tfrom['%s_TimestampTable' % dbkey]= (None,None,'%s.%s' % (ora_user, from_table), None, None,	None,None,None, ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,   0,0)
		tfrom['%s_TimestampTable_withHeader' % dbkey]= (None,None,'%s.%s' % (ora_user, from_table), None, None,None,None,None,	 ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,   1,0)
		tfrom['%s_TimestampTable_withComaHeader' % dbkey]= (None,None,'%s.%s' % (ora_user, from_table), None, None,	None, None,None,ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,   1,0)
		tfrom['%s_TimestampTable_keepWhitespace_withHeader' % dbkey]= (None,None,'%s.%s' % (ora_user, from_table), None, None, None,None,None, ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,   1,1)
		tfrom['%s_TimestampTable_trimWhitespace' % dbkey]= (None,None,'%s.%s' % (ora_user, from_table), None, None,	None,None,None, ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,   0,0)
		tfrom['%s_TimestampTable_keepWhitespace_Validate' % dbkey]= (None,None,'%s.%s' % (ora_user, from_table), None,None,None, None,	None, ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,   0,1)
		tfrom['%s_ShardedTimestampTable_keepWhitespace_Validate' % dbkey]= (None,None,'%s.%s' % (ora_user, from_table),None,None, None, None,None,	 ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,   0,1)
		tfrom['%s_DateTable' % dbkey]= (None,None,'%s.%s' % (ora_user, from_table), None, None,	None, None,None,ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,   0,0)
		tfrom['%s_DateTable_Email' % dbkey]= (None,None,'%s.%s' % (ora_user, from_table), None, None,	None,None,None, ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,   0,0)
		tfrom['%s_DateTable_JobName' % dbkey]= (None,None,'%s.%s' % (ora_user, from_table), None, None,	None,None,None, ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,   0,0)
		tfrom['%s_TimezoneTable' % dbkey]= (None,None,'%s.%s' % (ora_user, from_table), None, None,None,	None,None, ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,   0,0)
		tfrom['%s_TimezoneTable_KeepSpoolFile' % dbkey]= (None,None,'%s.%s' % (ora_user, from_table),None,None,None, None, None,	 ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,   0,0)
		tfrom['%s_Table_KeepSpoolFile' % dbkey]= 		 (None,None,'%s.%s' % (ora_user, from_table),None,None,None, None, None,	 ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,   0,0)
		...
		tfrom['%s_ShardedTable' % dbkey]= (None,None,'%s.%s' % (ora_user, from_table),None,None, None, None,	None, ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,   0,0)
		tfrom['%s_Parallel_ShardedTable' % dbkey]= (None,None,'%s.%s' % (ora_user, from_table),None,None, None, None,	None, ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,   0,0)
		...
		tfrom['%s_TableList' % dbkey]= (None,None,None, '%s.%s,%s.%s_2' % (ora_user,from_table,ora_user,from_table), None,	None,None,None, ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,   0,0)
		tfrom['%s_Parallel_TableList' % dbkey]= (None,None,None, '%s.%s,%s.%s_2' % (ora_user,from_table,ora_user,from_table), None,	None,None,None, ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,   0,0)
		
		if not dbkey in ['ORAXE']:
			tfrom['%s_Partition' % dbkey]= (None,None,'%s.%s' % (ora_user, from_part_table) , None, orapart,None,None, None, ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,   0,0)
			tfrom['%s_Partition_WithWideRows' % dbkey]= (None,None,'%s.%s' % (ora_user, from_part_table) ,None,  orapart,None, None, None,ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,   0,0)
			tfrom['%s_Partition_NoClient' % dbkey]= (None,None,'%s.%s' % (ora_user, from_part_table) , None, orapart,None,None,None, 	ora_user, ora_pwd, noclient_connect_string,nls_d, nls_t,  nls_tz,   0,0)
			tfrom['%s_Partition_TruncateTarget_AskToTruncate' % dbkey]= (None,None,'%s.%s' % (ora_user, from_part_table) ,None,  orapart,None,None,None, 	 ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,   0,0)
			tfrom['%s_Partition_withHeader' % dbkey]= (None,None,'%s.%s' % (ora_user, from_part_table) ,None,  orapart,None,None, None, 	 ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,   1,0)
			tfrom['%s_Partition_Validate' % dbkey]= (None,None,'%s.%s' % (ora_user, from_part_table) ,None, orapart,None,None,None, 	 ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,   0,0)
			tfrom['%s_Partition_keepWhitespace' % dbkey]= (None,None,'%s.%s' % (ora_user, from_part_table) , None, orapart,None,None,None,  ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,   0,1)
			tfrom['%s_Partition_trimWhitespace' % dbkey]= (None,None,'%s.%s' % (ora_user, from_part_table) ,None,  orapart,None,None,None, 	 ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,   0,0)
			tfrom['%s_Partition_KeepSpoolFile' % dbkey]= (None,None,'%s.%s' % (ora_user, from_part_table) , None, orapart,None,None,None, 	 ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,   0,0)
			...
			tfrom['%s_ShardedPartition' % dbkey]= (None,None,'%s.%s' % (ora_user, from_part_table) ,None,  orapart,None,None,	None,  ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,    0,0)
			...
			tfrom['%s_PartitionList' % dbkey]= (None,None,'%s.%s' % (ora_user, from_part_table) , None, None,'part_13,part_14,part_15,part_16' ,None, None, ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,   0,0)
			
			tfrom['%s_Parallel_PartitionList' % dbkey]= (None,None,'%s.%s' % (ora_user, from_part_table) , None, None,'part_13,part_14,part_15,part_16' ,None,None,  ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,   0,0)
			
			tfrom['%s_Subpartition' % dbkey]= (None,None,'%s.%s' % (ora_user, from_sub_part_table) ,None,  None,None, oraspart,None, 	ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,   0,0)
			tfrom['%s_Subpartition_WithWideRows' % dbkey]= (None,None,'%s.%s' % (ora_user, from_sub_part_table) ,None,  None,None, oraspart,	None, ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,   0,0)
			tfrom['%s_Subpartition_withHeader' % dbkey]= (None,None,'%s.%s' % (ora_user, from_sub_part_table) , None,None,None,  oraspart,None, 	ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,   1,0)
			tfrom['%s_Subpartition_Validate' % dbkey]= (None,None,'%s.%s' % (ora_user, from_sub_part_table) , None, None,None, oraspart,None, ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,   0,0)
			tfrom['%s_Subpartition_keepWhitespace' % dbkey]= (None,None,'%s.%s' % (ora_user, from_sub_part_table) , None, None,None, oraspart,None, 	ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,   0,1)
			tfrom['%s_Subpartition_KeepSpoolFile' % dbkey]= (None,None,'%s.%s' % (ora_user, from_sub_part_table) , None,None, None, oraspart,None, ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,    0,0)
			...
			tfrom['%s_ShardedSubpartition' % dbkey]= (None,None,'%s.%s' % (ora_user, from_sub_part_table),None,None,  None,oraspart,	None, ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,   0,0)
			...
			tfrom['%s_SubpartitionList' % dbkey]= (None,None,'%s.%s' % (ora_user, from_sub_part_table) ,None,  None,None, None,'%s,%s,%s,%s' % (oraspart,'part_15_sp2','part_14_sp1','part_14_sp2'), 	ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,   0,0)					
			tfrom['%s_Parallel_SubpartitionList' % dbkey]= (None,None,'%s.%s' % (ora_user, from_sub_part_table) ,None,  None,None, None,'%s,%s,%s,%s' % (oraspart,'part_10_sp2','part_14_sp1','part_14_sp2'), 	ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,   0,0)					
				
	dbkey='ORAXE'
	if 0:
		tfrom['ORAXE_QueryFile']= (qryfile,  None , None, None,	ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,  0,0)
		tfrom['ORAXE_QueryDir']= (None,  qry_dir_ora , None,  None,	ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,0,0)
		tfrom['ORAXE_ParallelQueryDir']= (None,  qry_dir_ora , None,  None,	ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz, 0,0)
		tfrom['ORAXE_DateTable']= (None,None,'%s.%s' % (ora_user, from_table), None, ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,0,0)
		tfrom['ORAXE_TimestampTable']= (None,None,'%s.%s' % (ora_user, from_table), None, ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,0,0)
		tfrom['ORAXE_Table']= (None,None,'%s.%s' % (ora_user, from_table), None,  ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz,0,0)
		tfrom['%s_TableList' % dbkey]= (None,None,None, '%s.%s,%s.%s_2' % (ora_user,from_table,ora_user,from_table), ora_user, ora_pwd, ora_sid,nls_d, nls_t, nls_tz,   0,0)	
		tfrom['ORAXE_TimezoneTable']= (None,None,'%s.%s' % (ora_user, from_table), None, ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz, 0,0)
		tfrom['ORAXE_Table_KeepSpoolFile']= (None,None,'%s.%s' % (ora_user, from_table),None,  ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz, 0,0)
		tfrom['ORAXE_ShardedTable']= (None,None,'%s.%s' % (ora_user, from_table), None,ora_user, ora_pwd, ora_sid,nls_d, nls_t,  nls_tz, 0,0)
	...
	#dbkey='SSENT'
	#print 'SSEXP','SS2012','SS2014','SS2016',	'SS70','SS2000', 'SS2005','SS2008',
	#e(0)
	#print conf.dbfam
	db_keys = [x for x,v in conf.dbfam.items() if v in ['SQL Server'] and not x in ['SSEXP']]
	#e(0)
	for dbkey in db_keys:
		ss_part='DateRange(Created)=3'
		tfrom['%s_QueryFile' % dbkey]= ( qryfile,None, None,None,	'sa', 'ssent_pwd', 'test',	'ALEX_BUZ-PC\MSSQLSERVER_ENT',	'"%s"' %	dbclients[dbkey])
		tfrom['%s_QueryFile_Limit15' % dbkey]= ( qryfile,None, None,None,	'sa', 'ssent_pwd', 'test',	'ALEX_BUZ-PC\MSSQLSERVER_ENT',	'"%s"' %	dbclients[dbkey])
		tfrom['%s_QueryDir' % dbkey]= ( None,qry_dir_ss, None,None,	'sa', 'ssent_pwd', 'test',	'ALEX_BUZ-PC\MSSQLSERVER_ENT',	'"%s"' %	dbclients[dbkey])
		tfrom['%s_QueryDir_Limit25' % dbkey]= ( None,qry_dir_ss, None,None,	'sa', 'ssent_pwd', 'test',	'ALEX_BUZ-PC\MSSQLSERVER_ENT',	'"%s"' %	dbclients[dbkey])
		
		tfrom['%s_ShardedQueryFile' % dbkey]= ( qryfile,None, None, None,	'sa', 'ssent_pwd', 'test',	'ALEX_BUZ-PC\MSSQLSERVER_ENT',	'"%s"' %	dbclients[dbkey])	
		tfrom['%s_Table' % dbkey]= ( None,None, from_table, None, 	'sa', 'ssent_pwd', 'test',	'ALEX_BUZ-PC\MSSQLSERVER_ENT',	'"%s"' %	dbclients[dbkey])
		tfrom['%s_Table_KeepSpoolFile' % dbkey]= ( None,None, from_table, None, 	'sa', 'ssent_pwd', 'test',	'ALEX_BUZ-PC\MSSQLSERVER_ENT',	'"%s"' %	dbclients[dbkey])
		tfrom['%s_Table_Limit10' % dbkey]= ( None,None, from_table, None, 	'sa', 'ssent_pwd', 'test',	'ALEX_BUZ-PC\MSSQLSERVER_ENT',	'"%s"' %	dbclients[dbkey])
		tfrom['%s_ShardedTable' % dbkey]= ( None, None,from_table, None,	'sa', 'ssent_pwd', 'test',	'ALEX_BUZ-PC\MSSQLSERVER_ENT',	'"%s"' %	dbclients[dbkey])	
		tfrom['%s_ShardedTable_Limit50' % dbkey]= ( None, None,from_table, None,	'sa', 'ssent_pwd', 'test',	'ALEX_BUZ-PC\MSSQLSERVER_ENT',	'"%s"' %	dbclients[dbkey])	
		if not  dbkey in ['SSEXP']:
			tfrom['%s_Partition' % dbkey]= ( None, None, from_part_table, ss_part,	'sa', 'ssent_pwd', 'test',	'ALEX_BUZ-PC\MSSQLSERVER_ENT',	'"%s"' %	dbclients[dbkey])
			tfrom['%s_Partition_Limit20' % dbkey]= ( None, None, from_part_table, ss_part,	'sa', 'ssent_pwd', 'test',	'ALEX_BUZ-PC\MSSQLSERVER_ENT',	'"%s"' %	dbclients[dbkey])
			tfrom['%s_ShardedPartition' % dbkey]= ( None,None, from_part_table, ss_part,	'sa', 'ssent_pwd', 'test',	'ALEX_BUZ-PC\MSSQLSERVER_ENT',	'"%s"' %	dbclients[dbkey])	
	...
	if citi:
		ora_user, ora_pwd, ora_sid=('CSMARTBSER', 'Tetris25', 'SMARTD1',)
		#to_part_table='TBL_PART_TABLE_TO'
		#to_sub_part_table='TBL_SUB_PART_TABLE_TO'
		noclient_connect_string="'(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=localhost)(PORT=1521))(CONNECT_DATA=(SERVER=DEDICATED)(SERVICE_NAME=orcl)))'"
	...
	to_orapart='part_13'
	for dbkey in ['ORA12C','ORA11G', 'ORAEXA','ORA10G','ORA9I','ORA8I','ORA733']:
		
		tto['%s_Table' % dbkey]=(ora_user, ora_pwd, ora_sid, '%s.%s' % (ora_user, to_table),None,None,nls_d, nls_t,nls_tz, 0)
		tto['%s_Table_NoTableName' % dbkey]=(ora_user, ora_pwd, ora_sid,None,None,None,nls_d, nls_t,nls_tz, 0)
		tto['%s_NoTable_TruncateTarget' % dbkey]=(ora_user, ora_pwd, ora_sid, None,None,None,nls_d, nls_t,nls_tz, 0)
		tto['%s_Table_SkipHeader' % dbkey]=(ora_user, ora_pwd, ora_sid, '%s.%s' % (ora_user, to_table),None,None,nls_d, nls_t,nls_tz, "1")
		...
		tto['%s_Table_DeleteTargetRecs' % dbkey]=(ora_user, ora_pwd, ora_sid,'%s.%s' % (ora_user, to_table),None,None,nls_d, nls_t,nls_tz, 0)
		#tto['%s_Table_WithWideRows' % dbkey]=(ora_user, ora_pwd, ora_sid, 'SCOTT.%s' % to_table,None,None,nls_d, nls_t,nls_tz, '"%s"' % dbclients[dbkey] )
		tto['%s_Table_NoClient' % dbkey]=(ora_user, ora_pwd, noclient_connect_string, '%s.%s' % (ora_user, to_table),None,None,nls_d, nls_t,nls_tz, 0  )
		tto['%s_Table_TruncateTarget' % dbkey]=(ora_user, ora_pwd, ora_sid, '%s.%s' % (ora_user, to_table),None,None,nls_d, nls_t,nls_tz, 0  )
		tto['%s_Table_TruncateTarget_NoClient' % dbkey]=(ora_user, ora_pwd, noclient_connect_string, '%s.%s' % (ora_user, to_table),None,None,nls_d, nls_t,nls_tz, 0  )
		tto['%s_Partition' % dbkey]=(ora_user, ora_pwd, ora_sid, '%s.%s' % (ora_user, to_part_table),to_orapart,None,nls_d, nls_t,nls_tz, 0  )
		#tto['%s_Partition_WithLoaderProfile' % dbkey]=(ora_user, ora_pwd, ora_sid, 'SCOTT.%s' % to_part_table,orapart,None,nls_d, nls_t,nls_tz  )
		tto['%s_Partition_TruncateTarget' % dbkey]=(ora_user, ora_pwd, ora_sid, '%s.%s' % (ora_user, to_part_table),orapart,None,nls_d, nls_t,nls_tz, 0  )
		tto['%s_Subpartition' % dbkey]=(ora_user, ora_pwd, ora_sid, '%s.%s' % (ora_user, to_table),None,oraspart,nls_d, nls_t,nls_tz, 0  )
		#tto['%s_Subpartition_WithLoaderProfile' % dbkey]=(ora_user, ora_pwd, ora_sid, 'SCOTT.%s' % to_table,None,oraspart,nls_d, nls_t,nls_tz  )
		tto['%s_Subpartition_TruncateTarget' % dbkey]=(ora_user, ora_pwd, ora_sid, '%s.%s' % (ora_user, to_sub_part_table),None,oraspart,nls_d, nls_t,nls_tz, 0  )
	if 0:
		for dbkey in ['ORA11G', 'ORAEXA']:
			tto['%s_Table' % dbkey]=(ora_user, ora_pwd, ora_sid,'%s.%s' % (ora_user, to_table),None,None,nls_d, nls_t,nls_tz, '"%s"' % dbclients[dbkey] )
			#tto['%s_Table_WithLoaderProfile' % dbkey]=(ora_user, ora_pwd, ora_sid, 'SCOTT.%s' % to_table,None,None,nls_d, nls_t,nls_tz, '"%s"' % dbclients[dbkey] )
			...
			tto['%s_Table_DeleteTargetRecs' % dbkey]=(ora_user, ora_pwd, ora_sid,'%s.%s' % (ora_user, to_table),None,None,nls_d, nls_t,nls_tz, '"%s"' % dbclients[dbkey] )
			#tto['%s_Table_WithWideRows' % dbkey]=(ora_user, ora_pwd, ora_sid, 'SCOTT.%s' % to_table,None,None,nls_d, nls_t,nls_tz, '"%s"' % dbclients[dbkey] )
			...
			tto['%s_Table_TruncateTarget' % dbkey]=(ora_user, ora_pwd, ora_sid,'%s.%s' % (ora_user, to_table),None,None,nls_d, nls_t,nls_tz, '"%s"' % dbclients[dbkey] )
			tto['%s_Table_TruncateTarget_NoClient' % dbkey]=(ora_user, ora_pwd, noclient_connect_string, '%s.%s' % (ora_user, to_table),None,None,nls_d, nls_t,nls_tz, '"%s"' % dbclients[dbkey] )
			tto['%s_Partition' % dbkey]=(ora_user, ora_pwd, ora_sid, '%s.%s' % (ora_user, to_part_table),orapart,None,nls_d, nls_t,nls_tz, '"%s"' % dbclients[dbkey] )
			...
			tto['%s_Partition_TruncateTarget' % dbkey]=(ora_user, ora_pwd, ora_sid, '%s.%s' % (ora_user, to_part_table),orapart,None,nls_d, nls_t,nls_tz, '"%s"' % dbclients[dbkey] )
			tto['%s_Subpartition' % dbkey]=(ora_user, ora_pwd, ora_sid, '%s.%s' % (ora_user, to_table),None,oraspart,nls_d, nls_t,nls_tz, '"%s"' % dbclients[dbkey] )
			...
			tto['%s_Subpartition_TruncateTarget' % dbkey]=(ora_user, ora_pwd, ora_sid, '%s.%s' % (ora_user, to_sub_part_table),None,oraspart,nls_d, nls_t,nls_tz, '"%s"' % dbclients[dbkey] )


		

	dbkey='ORAXE'		
	tto['%s_Table' % dbkey]=(ora_user, ora_pwd, ora_sid, '%s.%s' % (ora_user, to_table),nls_d, nls_t,nls_tz,0)	
	tto['%s_Table_NoTableName' % dbkey]=(ora_user, ora_pwd, ora_sid,None, nls_d, nls_t,nls_tz ,0 )	
	dbkey='MONGO'		
...
def run_test(db_from, db_to, prog, test,citi=False):
	...
	#pprint(par_keys)
	#test[pt]['CURL_DownloadFile']=[]
	#print conf.params['FROM'][paramkey_from]
	#for c in test[pt]:#[db_from]
	...
	print ''
	print '#'*50
	print "is_release=False,nor_t=(['%s'],['%s']),run=True)" % (db_from, db_to)
	print '#'*50
	...
	if 1:
		#print (conf._to)
		batdir=os.path.join(home,'bat')
		if not os.path.isdir(batdir):
			os.makedirs(batdir)
		scr= os.path.join(batdir, r'%s%s%s_test.bat' % (db_from.lower(),conf._to, db_to.lower()))
		print ''
		print scr
		f=open( scr,'w')
