...
class pnl_args(wx.Panel):
	...
	def __init__(self, parent, copy_vector,tmpl,the_id,args_api,size, style):
		...
		#self.changed=False
		self.tc_length=190
		self.tfile=os.path.join(home,'test','test_connnect','Oracle.sql')
		#self.Freeze()
		if 0: #Src Timer
			i=wx.NewId()			
		...
		if 1: #Source
			...
			for k,v in sorted(self.fargs.items()):
				...
				if k in ['input_files']:
					...
					image1 = wx.Image(imageFile, wx.BITMAP_TYPE_ANY).ConvertToBitmap()
					bitbtn = wx.BitmapButton(from_args_panel, id=-1, bitmap=image1,size = (image1.GetWidth()+6, image1.GetHeight()+6))
			...
			if not self.copy_vector[0].startswith('CSV'):
				...
				fgs.Add(tcbox, pos=(i, 1), flag=wx.TOP|wx.LEFT|wx.BOTTOM, border=1)
				btn.SetName('source')
		...
		if 1: #Target pnl
			...
			if not self.copy_vector[1].startswith('CSV'):	
				...
				imageFile = os.path.join(home,"images/editor_icon_16_2.png")
				image1 = wx.Image(imageFile, wx.BITMAP_TYPE_ANY).ConvertToBitmap()
				bitbtn = wx.BitmapButton(to_args_panel, id=-1, bitmap=image1,size = (image1.GetWidth()+6, image1.GetHeight()+6))
...
class DataBuddy(wx.Frame):
	def __init__(self, parent, id, title, size):
		...
		self.refreshType()
		sizer.Add(self.panel1, pos=(2, 0), span=(1,4),  flag=wx.EXPAND|wx.TOP|wx.LEFT|wx.RIGHT , border=1)	
			#self.gen_bind(wx.EVT_BUTTON,self.b_vector, self.OnVectorButton,('test'))				
