...
import shlex
class FromSybaseSQLAnywhere(SybaseSQLAnywhere):
	...
	def __init__(self,parent,log,datadir,conf,db):
		...
		SybaseSQLAnywhere.__init__(self,log,self.login,datadir,conf.args) 
		...
		if self.args.from_table:
			...
			if self.args.query_sql_file:
				...
				if self.lim:
					q='SELECT %s * FROM (%s) as t' % (self.lim,q)
...
From %s:	
	...
	def get_db_client_loc(self):
		...
		if self.args.source_client_home:
			self.db_client_loc=os.path.join(self.args.source_client_home.strip("'").strip('"').strip('\\').strip('\\'), loader)
		else:
