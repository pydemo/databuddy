# Title:	SSEXP_ShardedTable_Limit50 -->> SLITE_Table
# Description:	Arguments API for data-buddy.
__author__ = "Alex Buzunov, Sequelworks Inc."
__copyright__ = "Copyright 2015, data-buddy"
from args_api import args_api
api=args_api({'lame_duck': ('-l', '--lame_duck', 50, 'Limit rows (lame duck run).'), 'field_term': ('-t', '--field_term', '"|"', 'Field terminator.'), 'copy_vector': ('-w', '--copy_vector', 'ssexp2slite', 'Data copy direction.'), 'num_of_shards': ('-r', '--num_of_shards', 3, 'Number of shards.'), 'pool_size': ('-o', '--pool_size', 3, 'Pool size.')}, 
	{'from_db_name': ('-b', '--from_db_name', 'master', 'SQL Server Express source database.'), 'from_table': ('-c', '--from_table', 'Timestamp_test_from', 'From table.'), 'source_client_home': ('-z', '--source_client_home', '"C:\\Program Files\\Microsoft SQL Server\\Client SDK\\ODBC\\110\\Tools\\Binn"', 'Path to SQL Server Express client home.'), 'from_user': ('-j', '--from_user', 'sa', 'SQL Server Express source user.'), 'from_passwd': ('-x', '--from_passwd', '198Morgan', 'SQL Server Express source user password.'), 'from_db_server': ('-n', '--from_db_server', 'ALEX_BUZ-PC\\SQLEXPRESS', 'SQL Server Express source instance name.')}, 
