...
""" % (tab.upper(), schema.upper(), schema.upper(), tab.upper(),schema.upper(), tab.upper())
	...
	def spool_source_data_nt(self,outfn, spConf, payload):
		...
		output, err = p.communicate()
		#C:\Python27\data_migrator_1239_mongo\common\Tools\pscp.exe -pw oracle oracle@192.168.15.227:/tmp/qctest/pscp_test.zip C:\Python27\data_migrator_1239_mongo\CSV_OUT\20150813_204743_739000\pscp_test.zip.shard_0.data
		#scp.exe oracle@192.168.15.227:/tmp/qctest/* C:\Python27\data_migrator_1239_mongo\CSV_OUT\20150813_232725_423000
		
		#scp.exe scp.exe oracle@192.168.15.227:/tmp/qctest/
		
		#scp.exe oracle@192.168.15.227 ls
		
		print output
		
		if err.strip():
			print '#'*40
			for er in err.strip().split(os.linesep):
				print 'ERROR: |', er,'|'
				pprint(er)
			print '#'*40
		lines =[l for l in output.split('\n') if l]
		for line in lines:
			out_lines=line.strip().split('\r')
			#pprint (out_lines)
			size=out_lines[-1].split('|')[1]
			print 'Transferred: ',size
		#pprint(progress_lines)
		#last_line_status=progress_lines[-1].split(' ')
		#print last_line_status
		#if  self.dbg>1:
