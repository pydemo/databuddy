...
imp_file = os.path.join(home,'qc32','config','common_conf.py')
...
db_client_dbshell=None
db='ORA11G'
host_map_loc = r'".\config\host_map\host_map.py"'
#print host_map_loc
...
import include.v101.host_map as hmap
#print __dir__
print os.path.dirname( __file__)

print host_map_loc.strip('"').strip('.')
hm_path= os.path.join(home,'qc32',host_map_loc.strip('"').strip('.').strip(r'\\'))
#sys.exit(0)
hm = hmap.host_map([db,db],hm_path,0)
datadir=r'c:\tmp'
sqldir= r'%s\%s' % (datadir,'sql')
if not os.path.isdir(sqldir):
	os.mkdir(sqldir)
			
def save_qry(name, qry):
	assert datadir, 'Datadir is not set'	
	cqdir='%s\\sql' % (datadir)
	...
	#	print qry		
	return cqfn			
def get_db_client_dbshell():
	global db_client_dbshell
	if db_client_dbshell:
		return db_client_dbshell
	loader= os.path.basename(cconf.dbshell[db])
	if hm.local_source_client_home: #self.args.source_client_home:
		#print self.hm.local_source_client_home
		
		db_client_dbshell=(r'%s\%s' % (hm.local_source_client_home, loader)).strip("'").strip('\\').strip('\\')
		
	else:
		db_client_dbshell=cconf.dbshell[db]
		if not os.path.isfile(db_client_dbshell):
			log.warn('Path to source loader is not set. Defaulting to %' % loader)
	#print self.db_client_dbshell
	#e(0)
	return db_client_dbshell

		
def do_query(login, query, query_file=None, regexp=None, grp=None, spset='', qname=None, explain=0):
	status=0
	out=[]
	err=[]
	...
	if type(login) == types.TupleType:
		...
		login=login[0]
	q=''
	opt='set timing off pagesize 1000 line 2000 long 128000 feedback off colsep "|"'
	db_client_dbshell= get_db_client_dbshell()		
	...
	if query_file:
		...
		#print self.datadir
		q = "%s\n%s\nSELECT * FROM (%s) WHERE rownum<100;\nexit;" % ( spset,opt, query.strip().strip(';').strip())
		if explain:
			plan_id='test%s' % wx.NewId()
			print query
			q = "%s\n%s\nEXPLAIN PLAN  set statement_id='%s' FOR %s;\nSELECT PLAN_TABLE_OUTPUT FROM TABLE(DBMS_XPLAN.DISPLAY(NULL, '%s'));\nexit;\n" % ( spset,opt, plan_id, query.strip().strip(';').strip(), plan_id)
		print q
		assert qname, 'query name "qname" is not set'
		query_file=save_qry(qname,q)
		print query_file
		#q = "%s%s" % ( opt,query)
	...
	output=' '
	while output:
		output = p.stdout.readline() #string.replace(p2.stdout.readline(),'SQL>','')
		#blog.log(output,pos)
		#print output
		#print '#'*60
		if explain:
	...
	error= ' '
	while error:
		error = p.stderr.readline() #string.replace(p2.stdout.readline(),'SQL>','')
		#blog.log(output,pos)
		#print error
		#print '#'*60
		err.append(output)
...
try:
	...
	from wx import stc
	from include.StyledTextCtrl_sql import SqlSTC
	...
	class CodeEditor(SqlSTC):
		...
		def SetUpEditor(self):
			...
			"""
			import include.sql_keyword as keyword
...
class SimpleGrid(gridlib.Grid): ##, mixins.GridAutoEditMixin):
	...
	def UpdateLimitedGrid(self, sql,connect_as):
		...
		regexp=re.compile(r'(.*)')
		login = '%s/tiger@%s' % ( user,db)
		print login
		qname=r'test.sql'
		(all_resc, status,err)=do_query(login, query=sql,query_file=None,regexp=regexp,qname=qname)
		#print r_int[:10]
		#print len(all_resc)
		#print err	
		#sys.exit(0)
		...
		#print err	
		if err and err[0]: # or (not status and not cur.description) :
			#print(dir(d))
			print '#'*60
			print err
			print '#'*60
			...
			#headers=cur.description
			headers = [x.strip() for x in all_resc[1].strip().split('|') if x]
			print headers
			#sys.exit(0)
			if headers:
				print 'self.GetNumberCols()= ',self.GetNumberCols()
				...
				#print dir(cur)
				print 'fetch# %d' % i
				rs=[x.strip() for x in all_resc[3].strip().split('|') if x]
				pprint (rs)
				for d in range(len(headers)):
					print headers[d]
					self.SetColLabelValue(d, headers[d])
				...
				if 0:
					try:
						...
						status=1
						err=e	
				if 0: #err or err[0]: # or (not status and not cur.description) :
					#print(dir(d))
					print '#'*60
					print err
					print '#'*60
					...
					if rs:
						...
						while rs:
							...
							#rs=cur.fetchone()
							rs=[x.strip() for x in all_resc[2+i].strip().split('|') if x]
	...
	def UpdateLimitedGrid_NZ(self, sql,connect_as):
		#global d
		(db, user) = connect_as
		assert sql, 'sql statement is not defined'
		assert db, 'Database is not set'
...
class QueryPanel(wx.Panel):
	...
	def onTextKeyEvent(self, event):
		...
		if keycode==345: #F6 
			...
			plan_id='test%s' % wx.NewId()
			#q="EXPLAIN PLAN  set statement_id='%s' FOR %s" % (plan_id,sql)
			#print q
			regexp=re.compile(r'(.*)')
			(db, user) = self.connect_as()
			login = '%s/tiger@%s' % ( user,db)
			print login
			(all_resc, status,err)=do_query(login, query=sql,query_file=None,regexp=regexp,qname='plan', explain=True)
			#self.ExecSQL(q,self.connect_as())
			#print 1
			#print all_resc	
			#exq="SELECT PLAN_TABLE_OUTPUT FROM TABLE(DBMS_XPLAN.DISPLAY(NULL, '%s','BASIC'));" % plan_id
			
			#print exq
			#(all_resc, status,err)=do_query(login, query=q,query_file=None,regexp=regexp,qname='plan', explain=True)
			#print all_resc	
			self.parent.rp.pages[1][0].Status(''.join(all_resc))
