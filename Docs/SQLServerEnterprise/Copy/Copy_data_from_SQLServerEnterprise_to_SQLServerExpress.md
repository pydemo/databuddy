...
# Migrating SQLServerEnterprise table data to SQLServerExpress.
In this example we are going to migrate SQLServerEnterprise table data to SQLServerExpress.
Migration steps:
...
```  
(sample image)
![Define copy vector for SQLServerEnterprise-to-SQLServerExpress extract pipeline](https://raw.githubusercontent.com/data-buddy/DataBuddy/master/screenshots/MongoDB/Define_copy_vector_for_Oracle12c-to-MongoDB_copy_pipeline.png "Define copy vector for SQLServerEnterprise-to-SQLServerExpress copy pipeline.")

## Select Source and Target templates
Next and last step is template selection. Pick one from the source list on the left (`SSENT_TimestampTable`) and one from the target list on the right (`SSEXP_Table`):
(sample image)
...
```
############################################################
2015-06-01 11:30:09,022 - SSENT-SSEXP - INFO - Done.
2015-06-01 11:30:09,022 - SSENT-SSEXP - INFO - Elapsed: 00:00:00
