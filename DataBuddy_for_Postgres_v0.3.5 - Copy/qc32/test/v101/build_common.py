					...
					if '%s%s%s' % (paramkey_from.lower(),self.conf._to,db_to.lower()) not  in ('csv%scsv' % self.conf._to):
						...
						if 1:
							...
							if db_from.upper().split('_')[0] in self.conf.ff:
								...
								spool_type='csv'
							b=['%s%s%s' % (paramkey_from.lower(),self.conf._to,paramkey_to.lower()),1,			1,				'"%s"' % col_sep,		lame_duck,keep_spool_file,0,0,ask_to_truncate,0,email_to,log_dir,'',job_name,time_stamp,'',job_pre_etl,'','','',loader_profile,HostMap,spool_type,debug_level]
		...
		for t in tests:
			...
			for element in itertools.product(t_from,t_to):
				...
				#	print  '%s does not support limit' % conf.dbs[db_to.upper()]
					...
					print '2 passing on:', db_from, db_to					
				elif not 'TableNamedFiles' in db_from and 'GetTabnameFromQuery' in db_to:
					print '3 passing on:', db_from, db_to	
				elif  'Table_NoTableName' in db_to and not ('TableList' in db_from or db_from.endswith('Table')):
					print '4 passing on:', db_from, db_to,  'Table_NoTableName' in db_to, not 'TableList' in db_from				
				elif 'NoTable' in db_to and not 'NoTable' in db_from:
