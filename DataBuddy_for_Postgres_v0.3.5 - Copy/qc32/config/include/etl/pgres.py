...
class common(base):
	...
	def do_query(self,login, query, query_file=None, regexp=None, grp=None, spset='', qname=None):
		...
		if type(login) == types.TupleType:
			to_tab=login[1]
		login='c##test/scott@orcl12' #login[0]
		q=''
		opt=''
...
class source(common):
	...
	def get_db_client_dbshell(self):
		...
		if self.hm.local_source_client_home: #self.args.source_client_home:
			...
			self.db_client_dbshell=(r'%s\%s' % (self.hm.local_source_client_home.strip('"'), loader)).strip("'").strip('\\').strip('\\')
...
class target(common):
	...
	def get_db_client_dbshell(self):
		...
		if self.hm.local_target_client_home:
			self.db_client_dbshell=(r'%s\%s' % (self.hm.local_target_client_home.strip('"'), loader)).strip("'").strip('\\').strip('\\')
		else:
			self.db_client_dbshell=self.conf.dbtools['DBSHELL'][self.db]
	...
	def load_data_nt(self,logger,loadConf,outfn,shard):
		...
		#print  self.args.nls_timestamp_format
		#pprint(loadConf)
		 #e(0)
