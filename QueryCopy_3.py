...
class pnl_args(wx.Panel):
	...
	def open_session(self,copy_vector,tmpl,the_id,args):
		...
		if 1:
			...
			#pprint(self.obj.keys())
			self.parent.editor_panel.etl_loc={}
			self.parent.editor_panel.changed=False
			if self.obj.has_key(k):	
				#print 'has key'
...
class DataBuddy(wx.Frame):
	...
	def OnExit(self,e):
		#print self.changed
		if (self.changed or self.loader_panel.changed or self.editor_panel.Changed()) and self.if_yes('Do you want to save changes?','Application exit.'):
			send('save_args',())
			#(sname,cv,tmpl,dname,fname)=self.saveSession()
			#self.loader_panel.Save()
		else:
